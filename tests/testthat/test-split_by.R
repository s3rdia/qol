...
test_that("Split data frame by variable", {
    ...
    expect_true(!all(c(1, 2) %in% split_list[["NA_sex"]][["sex"]]))
})
...
test_that("Split data frame by variable and condition", {
    ...
    expect_type(split_list, "list")
    expect_equal(length(split_list), 4)

    expect_true(!2 %in% split_list[["1"]][["sex"]])
    expect_true(!1 %in% split_list[["2"]][["sex"]])
    expect_true(!all(c(1, 2) %in% split_list[["NA_sex"]][["sex"]]))
    expect_true(max(split_list[["age < 18"]][["age"]]) < 18)
})
...
test_that("Split data frame by variable using formats", {
    ...
    expect_equal(names(split_list), c("Total", "Male", "Female", "NA_sex"))
})
...
test_that("Split data frame throws a warning, if variable is not part of the data frame", {
    expect_message(split_list <- dummy_df |>
                       split_by("test"), " ! WARNING: The provided <conditions> variable 'test' is not part of")
})
