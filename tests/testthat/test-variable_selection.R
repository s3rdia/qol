...
test_that("Get variable names from beginning to position", {
    ...
    expect_equal(length(var_names), 6)
})
...
test_that("Providing not existing from-variable throws a warning", {
    expect_message(var_names <- dummy_df |> vars_between(test, person_id), " ! WARNING: 'from' variable '")
...
test_that("Providing more than one from or to variable throws a note", {
    ...
    expect_equal(length(var_names), 14)
})
