...
any_table <- function(data_frame,
    ...
    if (length(compute) > 0){
        ...
        new_var_names <- names(compute)

        # Create a regex pattern to be able to check all the extensions in one go
        extension_pattern <- paste0("(", paste(extensions, collapse = "|"), ")$")

        # Check which names end with any of the allowed extensions
        has_extension <- grepl(extension_pattern, new_var_names)

        # Compute new variables
        any_tab <- suppressMessages(any_tab |> compute.(compute))

        # If a variable name has a missing extension, add "_sum" to the name so
        # that the table header construction later on doesn't error.
        if (any(!has_extension)){
            # Add "_sum" to the names that are missing an extension
            extended_var_names <- new_var_names
            extended_var_names[!has_extension] <- paste0(new_var_names[!has_extension], "_sum")

            # Throw a warning listing the original names that were changed
            print_message("NOTE", c("The following variable[?s] generated by compute [?is/are] missing",
                                    "a <statistics> extension: [names]",
                                    "'_sum' is added as default, which influences the number format."),
                          names = new_var_names[!has_extension])

            # Get columns which match the newly generated variables and rename them
            matching_columns <- match(names(any_tab), new_var_names)
            names(any_tab)[!is.na(matching_columns)] <- extended_var_names[matching_columns[!is.na(matching_columns)]]
        }
