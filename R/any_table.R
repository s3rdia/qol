...
any_table <- function(data_frame,
    ...
    if (!order_by %in% c("values", "stats", "interleaved")){
        message(" ! WARNING: Order by option '", order_by, "' doesn't exist. Options 'values', 'stats' and 'interleaved'\n",
                "            are available.  Order by will be set to 'values'.")
        order_by <- "stats"
    }
    ...
    # In case of pre summarised data frame the additional "_sum" extension produced by
    # summarise_plus needs to be removed.
    if (pre_summed){
        names(any_tab) <- sub("_mean$", "", names(any_tab))
    }
