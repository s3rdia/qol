...
#'
#' @export
setup_nested_diagram_viewport <- function(arguments, diagram_info){
...
setup_diagram_viewport <- function(arguments){
    ...
    # Set up a new viewport for the whole diagram area to be able to safely work
    # in this area.
    setup_nested_viewport(x_pos       = dimensions[["diagram_start_left"]],
...
setup_legend <- function(diagram_info,
    ...
    # In case a present for horizontal fixed position was chosen, this one will
    # be preferred. It always can only be one preset active at a time.
    if (is.character(legend_x_pos) && legend_x_pos != "auto"){
        preset <- tolower(legend_x_pos)

        if (!preset %in% c("left", "right")){
            print_message("WARNING", "Horizontally only 'left' and 'right' preset available. 'left' will be used.")
            preset <- "left"
        }

        # On horizontal presets the y position gets fixed to the diagram top
        # starting position.
        legend_y_pos <- dimensions[["diagram_start_top"]] + margins

        # On the left side the x position is clear as it starts after the margin
        if (preset == "left"){
            legend_x_pos <- margins
        }
        # On the right side it cannot be determined at this stage, so here it is
        # set to 0 for easier calculation later on.
        else{
            legend_x_pos <- 0
        }
    }
    # If there is no horizontal preset chosen but a vertical one
    else if (is.character(legend_y_pos) && legend_y_pos != "auto"){
        preset <- legend_y_pos
        ...
        # On vertical presets the x position gets fixed to the diagrams left side
        # starting position.
        legend_x_pos <- margins

        # On the top the y position is clear as it starts at the set diagram start
        if (preset == "top"){
            legend_y_pos <- dimensions[["diagram_start_top"]] + margins
        }
        # On the bottom it cannot be determined at this stage, so here it is
        # set to 0 for easier calculation later on.
        else{
            legend_y_pos <- 0
        }
    ...
    for(column in seq_len(number_of_columns)){
        ...
        column_widths[column] <- get_text_width(longest_label, "label", dimensions, visuals, "cm") +
            visuals[["legend_symbol_size"]] + margins * 2
    }
    ...
    total_width  <- collapse::fsum(column_widths)

    if (legend_x_pos == "auto"){
        legend_x_pos <- dimensions[["graphic_width"]] - total_width
    }

    legend_x_pos <- legend_x_pos + column_x_pos[column_id]
    ...
    if (legend_y_pos == "auto"){
        legend_y_pos <- dimensions[["diagram_start_top"]] - text_height - margins
    }

    legend_y_pos <- legend_y_pos - (row_id - 1) * (text_height + margins / 2)

    # Check for presets and set up positions accordingly
    legend_symbol_size <- grid::unit(visuals[["legend_symbol_size"]], "cm")

    total_height <- 0

    if (!is.null(preset)){
        # y positions gets shifted down by text_height because it will be top adjusted
        # later on.
        legend_y_pos <- legend_y_pos - text_height

        # The total height is the difference between first and last row y position
        # with one additional text height and margin.
        total_height <- legend_y_pos[1] - legend_y_pos[length(legend_y_pos)] + text_height + margins

        # Depending on the preset the legends and diagrams position need to be adjusted
        if (preset == "right"){
            # Set the horizontal positions from the right. Since the x positions
            # started at 0 this calculation is pretty simple.
            legend_x_pos <- legend_x_pos + dimensions[["graphic_width"]] - total_width
            ...
            # The diagram start is shifted to the bottom by the legends total height.
            # The height shrinks accordingly.
    ...
    texts <- do.call(grid::gList, lapply(seq_len(number_of_labels), function(i){
        grid::textGrob(label  = segment_labels[i],
                       x      = grid::unit(legend_x_pos[i], "cm") + legend_symbol_size + grid::unit(margins, "cm"),
                       y      = grid::unit(legend_y_pos[i], "cm"),
                       just   = c("left", "top"),
    ...
    # Return legend and adjusted diagram dimensions
    list(grid::gList(rects, texts),
         dimensions[["diagram_start_left"]], dimensions[["diagram_start_top"]],
         dimensions[["diagram_width"]], dimensions[["diagram_height"]])
}
