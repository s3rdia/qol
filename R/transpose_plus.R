    ...
    if (long_to_wide){
        ...
        if (summarise || (!pre_summed && !is.null(formats))){
            ...
            if (!is.null(vars_per_stat_list)){
                variable_names <- setdiff(names(data_frame), c(group_vars, "TYPE", "TYPE_NR", "DEPTH"))
                ...
                data_frame <- suppressMessages(data_frame |> dropp(vars_to_drop))
            }

            data_frame <- suppressMessages(data_frame |> remove_stat_extension(statistics))
