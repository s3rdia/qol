...
#'
#' @export
dg_vertical_bars <- function(arguments, diagram_info){
    if (is.null(arguments) || !all(expected_parameters %in% names(arguments))){
        ...
        return(invisible(grid::nullGrob(name = "dg_vertical_bars")))
    }
    ...
    if (!arguments[["stacked"]]){
        segments <- vertical_grouped_bar_grob(diagram_info, arguments)
        ...
        segments <- vertical_stacked_bar_grob(diagram_info, arguments)
        diagram_info[["value_y_pos"]] <- segments[[2]]
        segments                      <- grid::gList(segments[[1]])

        # Set up segment labels
        segment_labels <- direct_horizontal_stacked_labels(diagram_info, arguments)
    }
