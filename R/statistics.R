...
#' grouping variables.
#'
#' @param missing_vector A logical vector which is TRUE for every observation
#' which is missing.
...
#'
#' @noRd
get_group_missings <- function(missing_vector, notes, na.rm){
    if (na.rm){
        return(FALSE)
    }
    ...
    missings <- collapse::fsum(missing_vector)

    # If there are missings calculate percentage and output a message
    if (missings > 0){
        nobs      <- length(missing_vector)
