...
expect_message(result_list <- dummy_df |>
    frequencies(variables = c(age, sex),
                by        = c(sex, year),
                print     = FALSE), " ! WARNING: The provided <by> variable '", info = "frequencies where by is also part of freq variables is aborted")
...
expect_message(result_list <- dummy_df |>
        frequencies(variables  = age,
                    ...
                    print_miss = TRUE,
                    means      = TRUE,
                    print      = FALSE), " ~ NOTE: Wenn <print_miss> is TRUE, there will be no mean tables. <Means> is set to FALSE.", info = "frequencies throws a NOTE, if print_miss and means option are both TRUE")
