...
dummy_df  <- suppressMessages(dummy_data(3000))
...
expect_message(result_list <- dummy_df |>
        any_table(rows    = "age",
                  ...
                  values  = weight,
                  by      = sex,
                  print   = FALSE), " X ERROR: The provided <by> variable 'sex' is also part of", info = "any_table with by variables aborts if by is also part of rows or columns")
...
expect_message(result_list <- dummy_df |>
                   any_table(rows    = "age",
                             ...
                             values  = weight,
                             output  = "Test",
                             print   = FALSE), " ! WARNING: <Output> format 'Test' not available.", info = "any_table warning with wrong output format")
...
expect_message(result_list <- dummy_df |>
                   any_table(rows     = "age",
                             ...
                             values   = weight,
                             order_by = "test",
                             print    = FALSE), " ! WARNING: <Order by> option 'test' doesn't exist", info = "any_table warning with wrong output format")
...
expect_message(result_list <- dummy_df |>
                   any_table(rows       = "age",
                             ...
                             print      = FALSE), " ! WARNING: <Statistic> 'test' is invalid and will be omitted.", info = "any_table throws a warning, if invalid statistic specified")
...
expect_message(result_list <- dummy_df |>
                   any_table(rows       = "age",
                             ...
                             values     = weight,
                             statistics = "test",
                             print      = FALSE), " ! WARNING: No valid <statistic> selected. 'sum' will be used.", info = "any_table throws a warning, if no valid statistic specified")
...
expect_message(result_list <- sum_df |>
       any_table(rows       = "year",
                 columns    = "sex",
                 values     = DEPTH,
                 print      = FALSE), " ! WARNING: All <values> variables need to have the <statistic> extension in their variable name.", info = "any_table throws a warning with missing statistic extension in pre summarised data")
...
expect_message(result_list <- dummy_df |>
                   any_table(rows       = "sex",
                             columns    = "sex",
                             statistics = c("sum"),
                             print      = FALSE), " X ERROR: The provided <columns> variable '", info = "any_table aborts, if column contains a row variable")
...
expect_message(result_list <- dummy_df |>
    any_table(rows    = "education",
              ...
              values  = weight,
              print   = FALSE), " X ERROR: Duplicate <columns> names found", info = "any_table aborts with duplicate column names after pivot")
...
expect_message(result_list <- dummy_df |>
      any_table(rows    = c("age", "age + test"),
                columns = "sex",
                values  = weight,
                print   = FALSE), " X ERROR: The provided <rows> variable 'test' is not part of", info = "any_table aborts with none existent row variable")
...
expect_message(result_list <- dummy_df |>
      any_table(rows    = "age",
                columns = c("sex + test"),
                values  = weight,
                print   = FALSE), " X ERROR: The provided <columns> variable 'test' is not part of", info = "any_table aborts with none existent column variable")
...
expect_message(result_list <- dummy_df |>
       any_table(rows    = "",
                 columns = "sex",
                 values  = weight,
                 print   = FALSE), " X ERROR: No valid <rows> variables provided", info = "any_table aborts with no valid row variables")
...
expect_message(result_list <- dummy_df |>
       any_table(rows    = "age",
                 ...
                 by      = "test",
                 values  = weight,
                 print   = FALSE), " ! WARNING: The provided <by> variable 'test' is not part of", info = "any_table aborts with invalid by variable")
...
expect_message(result_list <- dummy_df |>
       any_table(rows    = "age",
                 columns = "sex",
                 values  = "sex",
                 print   = FALSE), " x ERROR: The provided <rows>/<columns> variable 'sex' is also part of", info = "any_table aborts with row/column variable part of values")
...
expect_message(result_list <- dummy_df |>
       any_table(rows       = "age",
                 ...
                 print      = FALSE), " ! WARNING: Variable 'Test1' not found in the data frame.", info = "any_table outputs sum values with only invalid pct_value statistic and throws a warning")
...
expect_message(result_list <- sum_df2 |>
       any_table(rows    = c("year", "age"),
                 columns = "sex",
                 values  = weight_sum,
                 print   = FALSE), " X ERROR: The variable combination of '", info = "any_table aborts with missing variable combination in pre summarised data")
...
expect_message(result <- combine_into_workbook(1, file = temp_file),
               "X ERROR: Unknown object found. Provide <any_table> results.", info = "Combine tables into a single workbook aborts, if any not any_table object was found")
...
result_list <- withCallingHandlers(
    dummy_df |>
        any_table(rows       = "year",
                  ...
                  print      = FALSE),
    message = function(m) {
        errors <<- c(errors, conditionMessage(m))
...
expect_true(any(grepl("WARNING: Subsetting variable 'sex' by 'test' results in an empty data frame", errors)),
            info = "any_table aborts with no valid values after calculating the results")
expect_true(any(grepl("ERROR: After calculating the results, there are no valid values", errors)),
            info = "any_table aborts with no valid values after calculating the results")
