...
expect_message(result_df <- dummy_df |>
    crosstabs(rows    = c(age, state),
              columns = sex,
              print   = FALSE), " X ERROR: Only one variable for <rows> allowed. Crosstabs will be aborted.", info = "crosstabs not allowed with multiple row variables")
...
expect_message(result_df <- dummy_df |>
     crosstabs(rows    = sex,
               columns = c(age, state),
               print   = FALSE), " X ERROR: Only one variable for <columns> allowed. Crosstabs will be aborted.", info = "crosstabs not allowed with multiple column variables")
...
expect_message(result_list <- dummy_df |>
       crosstabs(rows    = age,
                 columns = sex,
                 by      = c(sex, year),
                 print   = FALSE), " ! WARNING: The provided <by> variable '", info = "crosstabs where by is also part of rows or columns is aborted")
...
expect_message(format_df <- dummy_df |>
      crosstabs(rows    = sex,
                ...
                print   = FALSE), " ~ NOTE: The format for variable 'sex' is a multilabel", info = "crosstabs row multilabel leads to notification")
...
expect_message(format_df <- dummy_df |>
       crosstabs(rows    = sex,
                 ...
                 formats = list(sex = suppressMessages(discrete_format(
                     ...
                     "Female" = 2))),
                 statistics = c("pct_row"),
                 print   = FALSE), " ~ NOTE: The format for variable 'sex' is a multilabel", info = "crosstabs row multilabel leads to notification with by variables")
...
expect_message(format_df <- dummy_df |>
       crosstabs(rows    = sex,
                 ...
                 statistics = c("sum", "freq", "pct_row", "pct_column", "pct_total"),
                 output  = "excel",
                 print   = FALSE), " ~ NOTE: The format for variable 'sex' is a multilabel", info = "crosstabs row multilabel leads to notification (excel)")
...
expect_message(format_df <- dummy_df |>
       crosstabs(rows    = sex,
                 ...
                 statistics = c("pct_row"),
                 output  = "excel",
                 print   = FALSE), " ~ NOTE: The format for variable 'sex' is a multilabel", info = "crosstabs row multilabel leads to notification with by variables (excel)")
...
expect_message(result_df <- dummy_df |>
       crosstabs(rows    = age,
                 ...
                 output  = "test",
                 print   = FALSE),
   " ! WARNING: <Output> format 'test' not available. Using 'console' instead.", info = "Invalid output format leads to console output")
