...
expect_message(split_list <- dummy_df |>
                   split_by("test"), " ! WARNING: The provided <conditions> variable 'test' is not part of", info = "Split data frame throws a warning, if variable is not part of the data frame")
...
expect_message(split_list <- dummy_df |>
      split_by("state", "age"), " X ERROR: Variable 'age' caused duplicate list entry names.", info = "Split data frame aborts, if duplicate list entry names are produced")


# Split data frame aborts, if no valid split element is passed
expect_message(split_list <- dummy_df |>
      split_by(1), " X ERROR: Only single variables or conditions allowed. Splitting will be aborted.", info = "Split data frame aborts, if no valid split element is passed")
