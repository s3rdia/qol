...
expect_message(dummy_df[["run_nr"]] <- dummy_df |> running_number(by = c(year, sex)),
               " ~ NOTE: Running number is generated in current data frame order.", info = "Generate running number with multiple by variables")
...
expect_message(dummy_df[["first"]] <- dummy_df |>
               ...
               " ~ NOTE: Cases are marked in current data frame order.", info = "Mark first and last cases with multiple by variables")
...
expect_message(dummy_df[["retain_value"]] <- dummy_df |>
                   retain_value(),
               " X ERROR: Must provide <values> to retain. Retain will be aborted.", info = "Retain value without providing a value")


# Retain sum without providing sum_of
expect_message(dummy_df[["retain_sum"]] <- dummy_df |>
                   retain_sum(),
               " X ERROR: Must provide a <values> to retain. Retain will be aborted.", info = "Retain sum without providing sum_of")


# Add new NA column range with wrong pattern returns original data frame
expect_message(retain_df <- (dummy_df |>
     retain_variables(status1:age3)), "X ERROR: Variable range has to be provided in the form 'var_name1:var_name10'.", info = "Add new NA column range with wrong pattern returns original data frame")
