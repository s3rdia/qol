...
value <- 100.123
expect_message(get_integer_length(value), " ! WARNING: Variable is not an integer and will be floored. Decimal places won't count.", info = "Warning in length of integer if numeric value provided")
...
expect_message(added_df <- dummy_df |>
     add_variable_range(status1:age3), " X ERROR: Variable range has to be provided in the form 'var_name1:var_name10'.", info = "Adding NA variables to data frame aborts on wrong pattern")
...
expect_message(added_df2 <- added_df |>
     add_variable_range(status1:status3), " X ERROR: Some variables are already part of the data frame: ", info = "Adding NA variables aborts, if variables are already part of data frame")


# Abort length of integer if character variable provided
expect_message(get_integer_length("Test"), " X ERROR: Only numeric values allowed.", info = "Abort length of integer if character variable provided")


# Abort round values if values are not numeric
expect_message(round_values("Test"), " X ERROR: Only numeric values allowed.", info = "Abort round values if values are not numeric")
