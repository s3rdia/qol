...
* any_table(): Can now output tables even though no column variables are specified.
...
* any_table(): Doesn't run into an error, if the table only consists of one value column.
